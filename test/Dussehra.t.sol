...
contract CounterTest is Test {
    ...
    function test_enterPeopleWhoLikeRam_notEqualFee() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Dussehra__NotEqualToEntranceFee.selector)
        );
    ...
    function test_increaseValuesOfParticipantsCorrectly() public participants {
        vm.startPrank(player1);
        choosingRam.increaseValuesOfParticipants(0, 1);
        vm.stopPrank();

        assertEq(ramNFT.getCharacteristics(1).isJitaKrodhah, true);
        assertEq(ramNFT.getCharacteristics(1).isDhyutimaan, false);
        assertEq(ramNFT.getCharacteristics(1).isVidvaan, false);
        assertEq(ramNFT.getCharacteristics(1).isAatmavan, false);
        assertEq(ramNFT.getCharacteristics(1).isSatyavaakyah, false);
    }

    function test_increaseValuesOfParticipantsToSelectRam()
        public
        participants
    {
        vm.startPrank(player1);
    ...
    function test_incrValuesOfParticipantsToSelectRam() public participants {
        vm.startPrank(player1);
        choosingRam.increaseValuesOfParticipants(0, 1);
        choosingRam.increaseValuesOfParticipants(0, 1);
        choosingRam.increaseValuesOfParticipants(0, 1);
    ...
    function test_OrganisersPaidkillRavana() public participants {
        vm.warp(1728691200 + 1);
        vm.startPrank(organiser);
        choosingRam.selectRamIfNotSelected();
        vm.stopPrank();
    ...
    function test_killRavanaIfTimeToBeLikeRamIsNotFinish() public participants {
        vm.expectRevert(
            abi.encodeWithSelector(
                ChoosingRam__TimeToBeLikeRamIsNotFinish.selector
            )
        );
        vm.startPrank(organiser);
        choosingRam.selectRamIfNotSelected();
    ...
    function test_killRavanaIfEventIsFinished() public participants {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(ChoosingRam__EventIsFinished.selector)
        );
        vm.startPrank(organiser);
        choosingRam.selectRamIfNotSelected();
        vm.stopPrank();
    ...
    function test_withdrawIfAlreadyClaimedAmount() public participants {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(Dussehra__AlreadyClaimedAmount.selector)
        );
        vm.startPrank(player2);
        dussehra.withdraw();
        vm.stopPrank();
