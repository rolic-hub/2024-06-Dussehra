...
contract Dussehra {
    ...
    modifier RamIsSelected() {
        require(
            choosingRamContract.isRamSelected(),
            "Ram is not selected yet!"
        );
        _;
    }
    ...
    modifier OnlyRam() {
        require(
            choosingRamContract.selectedRam() == msg.sender,
    ...
    constructor(
        ...
        address _choosingRamContract,
        address _ramNFT
    ) {
        entranceFee = _entranceFee;
        //The organiser is the deployer of the Dussehra contract
    ...
    //@audit - med: if ravanna is not killed on the specified day all the funds sendt to the contract will be stuck.
    //@audit - high: killRavanna can be called by anyone, and if called twice it wil transfer all the funds in 
    //the contract to the organiser.
    ...
    // //@audit - med/low: Re-entrancy vunerability
